'use client';
// src/app/admin/map/AdminMapView.tsx — 地圖主體（含側邊欄收合）
...
export default function AdminMapView() {
  ...
  const loadBuses = useCallback(async () => {
    try {
      ...
      else { const r2 = await fetch(`${API}/api/admin/buses`, { headers: H() }); data = await r2.json(); }
  ...
  function renderMarkers(data: any[]) {
    ...
    data.forEach((bus) => {
      ...
      else { const m = L.marker(ll, { icon }).on('click', () => selectBus(bus.id)); m.addTo(map); mkrs[bus.id] = m; }
    });
    if (bounds.length > 0 && !selBus) { try { map.fitBounds(bounds, { padding: [50, 50] }); } catch {} }
  }
  ...
  function selectBus(busId: number) {
    setBuses((prev) => {
      ...
      if (!bus) return prev;
      setSelBus(bus);
      if (bus.latitude && bus.longitude) mapRef.current?.setView([bus.latitude, bus.longitude], 15, { animate: true });
      return prev;
    });
    setShowPath(false);
    clearPath();
