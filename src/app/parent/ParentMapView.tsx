'use client';
// src/app/parent/ParentMapView.tsx — 家長地圖主體（垃圾車 app 風格）
...
const REFRESH_MS = 20000; // 20 秒自動刷新
...
export default function ParentMapView() {
  ...
  return (
    <div className="flex flex-col bg-gray-50" style={{ height: '100dvh', maxWidth: 500, margin: '0 auto' }}>
      ...
      {/* ── 底部面板 ── */}
      <div className="bg-white border-t border-gray-100 px-5 py-4 flex-shrink-0" style={{ boxShadow: '0 -4px 20px rgba(0,0,0,.06)' }}>
        ...
        {/* 路徑按鈕 */}
        {location && is_online && (
          <button onClick={togglePath} className={`w-full py-2 rounded-xl text-sm font-semibold mb-3 transition-all border ${showPath ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-200 text-gray-500 hover:border-blue-400 hover:text-blue-500'}`}>
